...
static void selectIfAvailable( QComboBox* box, const QString& itemText ) {
...
MainWindow::MainWindow( QWidget* parent )
  ...
  connect( ui->chk_debug,
           SIGNAL( clicked( bool ) ),
           SLOT( onDebugClicked( bool ) ) );
...
void MainWindow::onValueChanged() {
  ...
  if( !ui->chk_on->isChecked() || ( ui->cmbSerial->currentIndex() == 0 &&
  ...
  connect( bridge,
           SIGNAL( serialTraffic() ),
           ui->led_serial,
           SLOT( blinkOn() ) );
  bridge->attach(
    ui->cmbSerial->itemData( ui->cmbSerial->currentIndex() ).toString(),
...
void MainWindow::onDebugMessage( QString message ) {
  if( ui->chk_debug->isChecked() ) {
    onDisplayMessage( std::move( message ) );
  }
}
...
void MainWindow::setDebugFromCommandLine( bool showDebug ) {
  ui->chk_debug->setCheckState( showDebug ? Qt::CheckState::Checked :
