...
/*
 * Bridge is the actual controller that bridges Serial <-> MIDI. Create it by
 * passing the the serial & MIDI ports to use (or NULL if that part is not
 * used.)
 *
 * The Bridge manages its own internal state, and can be deleted when you stop
 * bridging and/or recreated with new settings.
 */
...
private:
  ...
  bool bufferStartsWith( uint8_t byte ) {
    return this->msg_data.length() && ( uint8_t )msg_data[ 0 ] == byte;
  }

  int
    running_status;  // if we have a running or current status byte, what is it?
