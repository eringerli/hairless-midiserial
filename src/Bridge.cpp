...
void Bridge::attach( const QString& serialName,
  ...
  if( serialName.length() && serialName != TEXT_NOT_CONNECTED ) {
    ...
    emit displayMessage(
      QString( "Opening serial port '%1'..." ).arg( serialName ) );
    this->serial = new QextSerialPort( serialName, serialSettings );
    connect( this->serial,
             SIGNAL( readyRead() ),
             this,
             SLOT( onSerialAvailable() ) );
  ...
  try {
    if( midiOutPort > -1 ) {
      emit displayMessage(
        QString( "Opening MIDI Out port #%1" ).arg( midiOutPort ) );
      this->midiOutPort = midiOutPort;
      this->midiOut     = new RtMidiOut( RtMidi::UNSPECIFIED, NAME_MIDI_OUT );
      this->midiOut->openPort( midiOutPort );
    }
  } catch( RtMidiError& e ) {
    emit displayMessage( QString( "Failed to open MIDI out port: %1" )
  ...
  try {
    if( midiInPort > -1 ) {
      emit displayMessage(
        QString( "Opening MIDI In port #%1" ).arg( midiInPort ) );
      this->midiInPort = midiInPort;
      this->midiIn     = new QRtMidiIn( NAME_MIDI_IN );
      this->midiIn->moveToThread( workerThread );
      this->midiIn->openPort( midiInPort );
      connect( this->midiIn,
               SIGNAL( messageReceived( double, QByteArray ) ),
               this,
               SLOT( onMidiIn( double, QByteArray ) ) );
    }
  } catch( RtMidiError& e ) {
    emit displayMessage( QString( "Failed to open MIDI in port: %1" )
                           .arg( QString::fromStdString( e.getMessage() ) ) );
  }
}
...
void Bridge::onMidiIn( double timeStamp, QByteArray message ) {
  ...
  emit    debugMessage(
    applyTimeStamp( QString( "MIDI In: %1" ).arg( description ) ) );
  emit midiReceived();
...
void Bridge::onStatusByte( uint8_t byte ) {
  ...
  if( this->data_expected == UNKNOWN_MIDI ) {
    emit displayMessage(
      applyTimeStamp( QString( "Warning: got unexpected status byte %1" )
                        .arg( byte, 0, 16 ) ) );
...
void Bridge::onDataByte( uint8_t byte ) {
  ...
  if( this->data_expected == 0 ) { // checking again just in in case running
                                   // status failed to update us to expect data
    emit displayMessage( applyTimeStamp(
      QString( "Error: got unexpected data byte 0x%1." ).arg( byte, 0, 16 ) ) );
    return;
  }
...
void Bridge::sendMidiMessage() {
  ...
  if( bufferStartsWith( MSG_DEBUG ) ) {
    ...
    emit displayMessage(
      applyTimeStamp( QString( "Serial Says: %1" ).arg( debug_msg ) ) );
  } else {
    emit debugMessage( applyTimeStamp(
      QString( "Serial In: %1" ).arg( describeMIDI( msg_data ) ) ) );
    if( midiOut ) {
      std::vector< uint8_t > message =
        std::vector< uint8_t >( msg_data.begin(), msg_data.end() );
...
QString Bridge::describeMIDI( QByteArray& buf ) {
  ...
  QString qdesc =
    QString( desc ).arg( ( int )channel + ( tag == TAG_SPECIAL ? 0 : 1 ) );

  if( tag == TAG_PITCH_BEND &&
      buf.length() == 3 ) { // recombine LSB/MSB for pitch bend
    return qdesc.arg( ( int )( ( uint8_t )buf[ 1 ] ) | ( ( uint8_t )buf[ 2 ] )
                                                         << 7 );
  }
...
QString Bridge::applyTimeStamp( const QString& message ) {
